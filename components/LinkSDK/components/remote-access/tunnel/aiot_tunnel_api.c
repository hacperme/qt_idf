...
static void _tunnel_event_callack(tunnel_manager_handle_t *tunnel_handle, aiot_tunnel_event_type type, char *tunnel_id)
{
    ...
    if(strlen(tunnel_id) <= sizeof(event.tunnel_id)) {
        memcpy(event.tunnel_id, tunnel_id, strlen(tunnel_id));
    }
...
static REMOTE_PROXY_INFO_S *_tunnel_create_new_tunnel(char *tunnel_id, aiot_tunnel_connect_param_t *params)
{
    ...
    if(strlen(tunnel_id) <= sizeof(remote_proxy_info->tunnel_id)) {
        memcpy(remote_proxy_info->tunnel_id, tunnel_id, strlen(tunnel_id));
    }

    if (params->host != NULL && strlen(params->host) <= HOST_LEN_MAX) {
    ...
    if (params->port != NULL && strlen(params->port) <= PORT_LEN_MAX) {
    ...
    if (params->path != NULL && strlen(params->path) <= 128) {
    ...
    if (params->token != NULL && strlen(params->token) <= IIOT_TUNNEL_TOKEN_LEN_MAX) {
