 ...
 * @param[in] handle tunnel管理模块句柄
 * @param[in] event tunnel消息结构体, 存放内部事件
 ...
 *
 * @param[in] handle tunnel管理模块句柄
 * @param[in] option 配置选项, 更多信息请参考@ref aiot_tunnel_option_t
 * @param[in] data   配置选项数据, 更多信息请参考@ref aiot_tunnel_option_t
 *
 * @return int32_t
 ...
 *
 * @param[in] handle 指向tunnel管理模块句柄的指针
 *
 * @return int32_t
 ...
 *
 * @param[in] handle 指向tunnel管理模块句柄的指针
 * @param[in] tunnel_id 隧道id
 * @param[in] params 隧道的建连信息
 *
 * @return int32_t*
 * @retval <STATE_SUCCESS 执行失败, 更多信息请参考 STATE_TUNNEL_* 定义
 * @retval >=STATE_SUCCESS 执行成功
